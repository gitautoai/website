...
Given `src/pages/checkout/index.tsx`, which test files should you look at? Sounds simple. It's not.

We build an AI agent that writes tests. Before the agent starts, we need to find existing test files so it can match the project's testing patterns. The agent wasted 20+ of its 30 iterations just reading files one by one - searching, guessing paths, retrying. So we decided to find and pre-load test files before the agent loop begins. The discovery algorithm is the hard part.

## Approach 1: Stem Matching

Extract the filename stem and search the tree. `src/auth/SessionProvider.tsx` has stem `SessionProvider`. Walk the file tree, find test files containing "SessionProvider" in their path. This works for most files.

It fails for generic stems. `src/pages/checkout/index.tsx` has stem `index`. Grepping for "index" across a codebase matches almost everything - 29 test files in one real repo. The signal drowns in noise.
...
Instead of matching paths, grep test file contents for the stem. If a test file imports `SessionProvider`, it references that implementation. This catches tests in completely different directories - a test in `src/pages/checkout/` might import `../../auth/SessionProvider`.
...
The problem shifts from discovery to ranking. A real repo produces 31 test file hits for `index.tsx`. Five of them are highly relevant (in `src/pages/checkout/` subtree). The other 26 are noise. Which 5 do we load into context?
...
Then we ran the ranker against 31 real file paths from a production repo. `src/index.test.tsx` (the root app test, completely unrelated) ranked #2. `src/pages/checkout/components/PayButton/index.test.tsx` (actually relevant) ranked #4.

The bug: `+30` was a flat bonus for any shared parent. One shared component (`src/`) got the same +30 as three shared components (`src/pages/checkout/`). With 3 synthetic inputs, other scoring factors dominated. With 31 real inputs at varying depths, the flat bonus broke everything.
