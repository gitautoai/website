  ...
  test("should verify auto-reload payment processing", async () => {
    ...
    await test.step("Verify database updates", async () => {
      ...
      if (newTransactions?.length !== 1) {
        console.log(`Expected 1 auto_reload transaction, got ${newTransactions?.length}`);
        console.log(
          "Transactions:",
      ...
      expect(newTransactions?.length).toBe(1); // Should have exactly 1 auto_reload transaction

      const autoReloadTransaction = newTransactions?.[0];
      expect(autoReloadTransaction?.amount_usd).toBe(45); // Should be exactly 50 - 5 = 45
      expect(autoReloadTransaction?.stripe_payment_intent_id).toBeTruthy();
    });
    } finally {
      // Cleanup test data
  ...
  test("should verify cron job processes multiple owners", async () => {
    ...
    try {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (!ownerResult.testOwnerId || !ownerResult.testCustomerId) throw new Error(`Failed to get test owner data for owner ${i}`);
